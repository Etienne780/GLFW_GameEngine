...
struct EngineWindow;                 // Storage for one window
...
struct ImGuiPopupData
{
    ...
    EngineWindow*        Window;         // Resolved on BeginPopup() - may stay unresolved if user never calls OpenPopup()
    EngineWindow*        RestoreNavWindow;// Set on OpenPopup(), a NavWindow that will be restored on popup close
...
{
    EngineWindow*        Window;         // Init,Move    // Best candidate window (result->ItemWindow->RootWindowForNav == request->Window)
...
struct IMGUI_API ImGuiDockNode
{
    ...
    ImVector<EngineWindow*>  Windows;                    // Note: unordered list! Iterate TabBar->Tabs for user-order.
    ImGuiTabBar*            TabBar;
    ...
    EngineWindow*            HostWindow;
    EngineWindow*            VisibleWindow;              // Generally point to window which is ID is == SelectedTabID, but when CTRL+Tabbing this can be a different window.
...
{
    EngineWindow*        Window;                 // Set when the viewport is owned by a window (and ImGuiViewportFlags_CanHostOtherWindows is NOT set)
    int                 Idx;
...
struct ImGuiContext
{
    ...
    // Windows state
    ImVector<EngineWindow*>  Windows;                            // Windows, sorted in display order, back to front
    ImVector<EngineWindow*>  WindowsFocusOrder;                  // Root windows, sorted in focus order, back to front.
    ImVector<EngineWindow*>  WindowsTempSortBuffer;              // Temporary buffer used in EndFrame() to reorder windows so parents are kept before their child
    ...
    EngineWindow*            CurrentWindow;                      // Window being drawn into
    EngineWindow*            HoveredWindow;                      // Window the mouse is hovering. Will typically catch mouse inputs.
    EngineWindow*            HoveredWindowUnderMovingWindow;     // Hovered window ignoring MovingWindow. Only set if MovingWindow is set.
    EngineWindow*            HoveredWindowBeforeClear;           // Window the mouse is hovering. Filled even with _NoMouse. This is currently useful for multi-context compositors.
    EngineWindow*            MovingWindow;                       // Track the window we clicked on (in order to preserve focus). The actual window that is moved is generally MovingWindow->RootWindowDockTree.
    EngineWindow*            WheelingWindow;                     // Track the window we started mouse-wheeling on. Until a timer elapse or mouse has moved, generally keep scrolling the same window even if during the course of scrolling the mouse ends up hovering a child window.
    ...
    EngineWindow*            NavWindow;                          // Focused window for navigation. Could be called 'FocusedWindow'
    ...
    EngineWindow*            NavWindowingTarget;                 // Target window when doing CTRL+Tab (or Pad Menu + FocusPrev/Next), this window is temporarily displayed top-most!
    EngineWindow*            NavWindowingTargetAnim;             // Record of last valid NavWindowingTarget until DimBgRatio and NavWindowingHighlightAlpha becomes 0.0f, so the fade-out can stay on it.
    EngineWindow*            NavWindowingListWindow;             // Internal window actually listing the CTRL+Tab contents
    ...
    EngineWindow*            TooltipPreviousWindow;              // Window of last tooltip submitted during the frame
...
struct IMGUI_API ImGuiWindowTempData
{
    ...
    ImVector<EngineWindow*>  ChildWindows;
...
struct IMGUI_API EngineWindow
{
    ...
    EngineWindow*            ParentWindow;                       // If we are a child _or_ popup _or_ docked window, this is pointing to our parent. Otherwise NULL.
    EngineWindow*            ParentWindowInBeginStack;
    EngineWindow*            RootWindow;                         // Point to ourself or first ancestor that is not a child window. Doesn't cross through popups/dock nodes.
    EngineWindow*            RootWindowPopupTree;                // Point to ourself or first ancestor that is not a child window. Cross through popups parent<>child.
    EngineWindow*            RootWindowDockTree;                 // Point to ourself or first ancestor that is not a child window. Cross through dock nodes.
    EngineWindow*            RootWindowForTitleBarHighlight;     // Point to ourself or first ancestor which will display TitleBgActive color when this window is active.
    EngineWindow*            RootWindowForNav;                   // Point to ourself or first ancestor which doesn't have the NavFlattened flag.
    EngineWindow*            ParentWindowForFocusRoute;          // Set to manual link a window to its logical parent so that Shortcut() chain are honoerd (e.g. Tool linked to Document)

    EngineWindow*            NavLastChildNavWindow;              // When going to the menu bar, we remember the child window we came from. (This could probably be made implicit if we kept g.Windows sorted by last focused including child window.)
...
public:
    EngineWindow(ImGuiContext* context, const char* name);
    ~EngineWindow();
...
struct ImGuiTabItem
{
    ...
    EngineWindow*        Window;                 // When TabItem is part of a DockNode's TabBar, we hold on to a window.
...
struct IMGUI_API ImGuiTable
{
    ...
    EngineWindow*                OuterWindow;                // Parent window for the table
    EngineWindow*                InnerWindow;                // Window holding the table data (== OuterWindow or a child window)
...
namespace ImGui
{
    ...
    inline    EngineWindow*  GetCurrentWindowRead()      { ImGuiContext& g = *GImGui; return g.CurrentWindow; }
    inline    EngineWindow*  GetCurrentWindow()          { ImGuiContext& g = *GImGui; g.CurrentWindow->WriteAccessed = true; return g.CurrentWindow; }
    IMGUI_API EngineWindow*  FindWindowByID(ImGuiID id);
    IMGUI_API EngineWindow*  FindWindowByName(const char* name);
    IMGUI_API void          UpdateWindowParentAndRootLinks(EngineWindow* window, ImGuiWindowFlags flags, EngineWindow* parent_window);
    IMGUI_API void          UpdateWindowSkipRefresh(EngineWindow* window);
    IMGUI_API ImVec2        CalcWindowNextAutoFitSize(EngineWindow* window);
    IMGUI_API bool          IsWindowChildOf(EngineWindow* window, EngineWindow* potential_parent, bool popup_hierarchy, bool dock_hierarchy);
    IMGUI_API bool          IsWindowWithinBeginStackOf(EngineWindow* window, EngineWindow* potential_parent);
    IMGUI_API bool          IsWindowAbove(EngineWindow* potential_above, EngineWindow* potential_below);
    IMGUI_API bool          IsWindowNavFocusable(EngineWindow* window);
    IMGUI_API void          SetWindowPos(EngineWindow* window, const ImVec2& pos, ImGuiCond cond = 0);
    IMGUI_API void          SetWindowSize(EngineWindow* window, const ImVec2& size, ImGuiCond cond = 0);
    IMGUI_API void          SetWindowCollapsed(EngineWindow* window, bool collapsed, ImGuiCond cond = 0);
    IMGUI_API void          SetWindowHitTestHole(EngineWindow* window, const ImVec2& pos, const ImVec2& size);
    IMGUI_API void          SetWindowHiddenAndSkipItemsForCurrentFrame(EngineWindow* window);
    inline void             SetWindowParentWindowForFocusRoute(EngineWindow* window, EngineWindow* parent_window) { window->ParentWindowForFocusRoute = parent_window; } // You may also use SetNextWindowClass()'s FocusRouteParentWindowId field.
    inline ImRect           WindowRectAbsToRel(EngineWindow* window, const ImRect& r) { ImVec2 off = window->DC.CursorStartPos; return ImRect(r.Min.x - off.x, r.Min.y - off.y, r.Max.x - off.x, r.Max.y - off.y); }
    inline ImRect           WindowRectRelToAbs(EngineWindow* window, const ImRect& r) { ImVec2 off = window->DC.CursorStartPos; return ImRect(r.Min.x + off.x, r.Min.y + off.y, r.Max.x + off.x, r.Max.y + off.y); }
    inline ImVec2           WindowPosAbsToRel(EngineWindow* window, const ImVec2& p)  { ImVec2 off = window->DC.CursorStartPos; return ImVec2(p.x - off.x, p.y - off.y); }
    inline ImVec2           WindowPosRelToAbs(EngineWindow* window, const ImVec2& p)  { ImVec2 off = window->DC.CursorStartPos; return ImVec2(p.x + off.x, p.y + off.y); }

    // Windows: Display Order and Focus Order
    IMGUI_API void          FocusWindow(EngineWindow* window, ImGuiFocusRequestFlags flags = 0);
    IMGUI_API void          FocusTopMostWindowUnderOne(EngineWindow* under_this_window, EngineWindow* ignore_window, ImGuiViewport* filter_viewport, ImGuiFocusRequestFlags flags);
    IMGUI_API void          BringWindowToFocusFront(EngineWindow* window);
    IMGUI_API void          BringWindowToDisplayFront(EngineWindow* window);
    IMGUI_API void          BringWindowToDisplayBack(EngineWindow* window);
    IMGUI_API void          BringWindowToDisplayBehind(EngineWindow* window, EngineWindow* above_window);
    IMGUI_API int           FindWindowDisplayIndex(EngineWindow* window);
    IMGUI_API EngineWindow*  FindBottomMostVisibleWindowWithinBeginStack(EngineWindow* window);
    ...
    inline ImDrawList*      GetForegroundDrawList(EngineWindow* window) { return GetForegroundDrawList(window->Viewport); }
    ...
    IMGUI_API void          FindHoveredWindowEx(const ImVec2& pos, bool find_first_and_in_any_viewport, EngineWindow** out_hovered_window, EngineWindow** out_hovered_window_under_moving_window);
    IMGUI_API void          StartMouseMovingWindow(EngineWindow* window);
    IMGUI_API void          StartMouseMovingWindowOrNode(EngineWindow* window, ImGuiDockNode* node, bool undock);
    ...
    IMGUI_API void          SetWindowViewport(EngineWindow* window, ImGuiViewportP* viewport);
    IMGUI_API void          SetCurrentViewport(EngineWindow* window, ImGuiViewportP* viewport);
    ...
    IMGUI_API void                  MarkIniSettingsDirty(EngineWindow* window);
    ...
    IMGUI_API ImGuiWindowSettings*  FindWindowSettingsByWindow(EngineWindow* window);
    ...
    // Scrolling
    IMGUI_API void          SetScrollX(EngineWindow* window, float scroll_x);
    IMGUI_API void          SetScrollY(EngineWindow* window, float scroll_y);
    IMGUI_API void          SetScrollFromPosX(EngineWindow* window, float local_x, float center_x_ratio);
    IMGUI_API void          SetScrollFromPosY(EngineWindow* window, float local_y, float center_y_ratio);

    // Early work-in-progress API (ScrollToItem() will become public)
    IMGUI_API void          ScrollToItem(ImGuiScrollFlags flags = 0);
    IMGUI_API void          ScrollToRect(EngineWindow* window, const ImRect& rect, ImGuiScrollFlags flags = 0);
    IMGUI_API ImVec2        ScrollToRectEx(EngineWindow* window, const ImRect& rect, ImGuiScrollFlags flags = 0);
//#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    inline void             ScrollToBringRectIntoView(EngineWindow* window, const ImRect& rect) { ScrollToRect(window, rect, ImGuiScrollFlags_KeepVisibleEdgeY); }
//#endif
    ...
    IMGUI_API void          SetActiveID(ImGuiID id, EngineWindow* window);
    IMGUI_API void          SetFocusID(ImGuiID id, EngineWindow* window);
    ...
    IMGUI_API bool          IsWindowContentHoverable(EngineWindow* window, ImGuiHoveredFlags flags = 0);
    ...
    IMGUI_API void          ClosePopupsOverWindow(EngineWindow* ref_window, bool restore_focus_to_window_under_popup);
    IMGUI_API void          ClosePopupsExceptModals();
    IMGUI_API bool          IsPopupOpen(ImGuiID id, ImGuiPopupFlags popup_flags);
    IMGUI_API ImRect        GetPopupAllowedExtentRect(EngineWindow* window);
    IMGUI_API EngineWindow*  GetTopMostPopupModal();
    IMGUI_API EngineWindow*  GetTopMostAndVisiblePopupModal();
    IMGUI_API EngineWindow*  FindBlockingModal(EngineWindow* window);
    IMGUI_API ImVec2        FindBestWindowPosForPopup(EngineWindow* window);
    ...
    // Keyboard/Gamepad Navigation
    IMGUI_API void          NavInitWindow(EngineWindow* window, bool force_reinit);
    ...
    IMGUI_API void          NavMoveRequestTryWrapping(EngineWindow* window, ImGuiNavMoveFlags move_flags);
    IMGUI_API void          NavHighlightActivated(ImGuiID id);
    IMGUI_API void          NavClearPreferredPosForAxis(ImGuiAxis axis);
    IMGUI_API void          SetNavCursorVisibleAfterMove();
    IMGUI_API void          NavUpdateCurrentWindowIsScrollPushableX();
    IMGUI_API void          SetNavWindow(EngineWindow* window);
    ...
    IMGUI_API void          DockContextQueueDock(ImGuiContext* ctx, EngineWindow* target, ImGuiDockNode* target_node, EngineWindow* payload, ImGuiDir split_dir, float split_ratio, bool split_outer);
    IMGUI_API void          DockContextQueueUndockWindow(ImGuiContext* ctx, EngineWindow* window);
    IMGUI_API void          DockContextQueueUndockNode(ImGuiContext* ctx, ImGuiDockNode* node);
    IMGUI_API void          DockContextProcessUndockWindow(ImGuiContext* ctx, EngineWindow* window, bool clear_persistent_docking_ref = true);
    IMGUI_API void          DockContextProcessUndockNode(ImGuiContext* ctx, ImGuiDockNode* node);
    IMGUI_API bool          DockContextCalcDropPosForDocking(EngineWindow* target, ImGuiDockNode* target_node, EngineWindow* payload_window, ImGuiDockNode* payload_node, ImGuiDir split_dir, bool split_outer, ImVec2* out_pos);
    ...
    IMGUI_API bool          GetWindowAlwaysWantOwnTabBar(EngineWindow* window);
    IMGUI_API void          BeginDocked(EngineWindow* window, bool* p_open);
    IMGUI_API void          BeginDockableDragDropSource(EngineWindow* window);
    IMGUI_API void          BeginDockableDragDropTarget(EngineWindow* window);
    IMGUI_API void          SetWindowDock(EngineWindow* window, ImGuiID dock_id, ImGuiCond cond);
    ...
    // Box-Select API
    IMGUI_API bool          BeginBoxSelect(const ImRect& scope_rect, EngineWindow* window, ImGuiID box_select_id, ImGuiMultiSelectFlags ms_flags);
    ...
    IMGUI_API void          SetWindowClipRectBeforeSetChannel(EngineWindow* window, const ImRect& clip_rect);
    ...
    IMGUI_API ImGuiOldColumns* FindOrCreateColumns(EngineWindow* window, ImGuiID id);
    ...
    IMGUI_API void          TabBarAddTab(ImGuiTabBar* tab_bar, ImGuiTabItemFlags tab_flags, EngineWindow* window);
    ...
    IMGUI_API bool          TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open, ImGuiTabItemFlags flags, EngineWindow* docked_window);
    IMGUI_API void          TabItemSpacing(const char* str_id, ImGuiTabItemFlags flags, float width);
    IMGUI_API ImVec2        TabItemCalcSize(const char* label, bool has_close_button_or_unsaved_marker);
    IMGUI_API ImVec2        TabItemCalcSize(EngineWindow* window);
    ...
    IMGUI_API ImRect        GetWindowScrollbarRect(EngineWindow* window, ImGuiAxis axis);
    IMGUI_API ImGuiID       GetWindowScrollbarID(EngineWindow* window, ImGuiAxis axis);
    IMGUI_API ImGuiID       GetWindowResizeCornerID(EngineWindow* window, int n); // 0..3: corners
    IMGUI_API ImGuiID       GetWindowResizeBorderID(EngineWindow* window, ImGuiDir dir);
    ...
    IMGUI_API void          GcCompactTransientWindowBuffers(EngineWindow* window);
    IMGUI_API void          GcAwakeTransientWindowBuffers(EngineWindow* window);
    ...
    IMGUI_API void          DebugNodeDrawList(EngineWindow* window, ImGuiViewportP* viewport, const ImDrawList* draw_list, const char* label);
    ...
    IMGUI_API void          DebugNodeWindow(EngineWindow* window, const char* label);
    IMGUI_API void          DebugNodeWindowSettings(ImGuiWindowSettings* settings);
    IMGUI_API void          DebugNodeWindowsList(ImVector<EngineWindow*>* windows, const char* label);
    IMGUI_API void          DebugNodeWindowsListByBeginStackParent(EngineWindow** windows, int windows_size, EngineWindow* parent_in_begin_stack);
